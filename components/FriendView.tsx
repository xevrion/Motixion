...
export const FriendView: React.FC = () => {
  ...
  const handleAcceptFromProfile = async () => {
    ...
    try {
      await friendService.acceptFriendRequest(pendingRequest.id);
      setPendingRequest(null);
  ...
  const handleRejectFromProfile = async () => {
    if (!pendingRequest) return;
    try {
      await friendService.rejectFriendRequest(pendingRequest.id);
      setPendingRequest(null);
      setLeaderboardView(true);
      await loadFriendRequests();
    } catch (error) {
  ...
  const handleRemoveFriend = async () => {
    if (!selectedFriend?.friendship_id) return;
    if (!confirm(`Are you sure you want to remove ${selectedFriend.username} from your friends?`)) return;
    try {
      await friendService.removeFriend(selectedFriend.friendship_id);
  ...
  const handleCancelRequest = async () => {
    if (!pendingRequest) return;
    if (!confirm(`Are you sure you want to cancel your friend request to ${selectedFriend?.username}?`)) return;
    try {
      await friendService.removeFriend(pendingRequest.id);
      setPendingRequest(null);
      setSelectedFriend(null);
      setLeaderboardView(true);
      await loadFriendRequests();
  ...
  // Profile view (must be checked before "no friends" view to allow viewing pending request profiles)
    ...
    return (
      <div className="space-y-6 md:space-y-8">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          {pendingRequest && pendingRequest.friend_id === user.id && (
            <div className="flex gap-3 w-full justify-center sm:justify-end">
              <button
                onClick={handleAcceptFromProfile}
                className="flex items-center gap-2 px-4 py-2 bg-emerald-500 text-white rounded-xl hover:bg-emerald-400 transition-colors font-medium text-sm sm:text-base w-1/2 sm:w-auto justify-center"
              >
                <Check size={18} />
                Accept
              </button>
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-rose-500 text-white rounded-xl hover:bg-rose-400 transition-colors font-medium text-sm sm:text-base w-1/2 sm:w-auto justify-center"
              >
                <X size={18} />
                Reject
              </button>
          ...
          {pendingRequest && pendingRequest.user_id === user.id && (
            <button
              onClick={handleCancelRequest}
              className="flex items-center gap-2 px-4 py-2 bg-zinc-200 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 rounded-xl hover:bg-rose-500 hover:text-white transition-colors font-medium text-sm sm:text-base"
            >
              <X size={18} />
              Cancel Request
            </button>
          ...
          {!pendingRequest && selectedFriend?.friendship_id && (
            <button
          ...
          <div className="space-y-4 sm:space-y-6">
            <div className="bg-white dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-800 p-4 sm:p-6 md:p-8 rounded-2xl flex flex-col items-center text-center">
              ...
              <p className="text-zinc-500 text-xs sm:text-sm mb-6">
                {pendingRequest 
                  ? (pendingRequest.friend_id === user.id ? "Incoming Request" : "Request Sent")
            ...
            <div className="bg-white dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-800 p-4 sm:p-6 rounded-2xl">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-zinc-900 dark:text-white font-bold">
                    {selectedFriend.current_streak || 0} days
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-zinc-600 dark:text-zinc-400">
            ...
            {pendingRequest ? (
              <div className="flex-1 min-h-[250px] sm:min-h-[300px] flex items-center justify-center">
                <div className="text-center text-zinc-500">
                  ...
                  <p className="text-lg font-medium mb-2">Activity data hidden</p>
                  <p className="text-sm">Accept the friend request to view their full activity history</p>
                </div>
              </div>
            ) : (
            ...
            <div className="space-y-3">
              {friendRequests.map((request) => {
                ...
                return (
                  <div
                    key={request.id}
                    onClick={() => viewRequesterProfile(request)}
                    className="flex items-center justify-between bg-zinc-100 dark:bg-zinc-950 p-3 sm:p-4 rounded-xl gap-3 cursor-pointer hover:bg-zinc-200 dark:hover:bg-zinc-900 transition-colors"
                  >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-3 mt-0.5">
                          <span className="text-xs text-zinc-500 flex items-center gap-1">
                            ...
                            {requester.current_streak || 0} streak
                          </span>
                          <span className="text-xs text-zinc-500">
                            {requester.balance || 0} pts
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    {!isIncoming && (
                      <span className="text-xs text-zinc-500 italic flex-shrink-0">Pending</span>
                    )}
                  </div>
                );
              })}
            ...
            {friendRequests.map((request) => {
              ...
              return (
                <div
                  key={request.id}
                  onClick={() => viewRequesterProfile(request)}
                  className="flex items-center justify-between bg-zinc-100 dark:bg-zinc-950/50 p-3 sm:p-4 rounded-xl gap-3 cursor-pointer hover:bg-zinc-200 dark:hover:bg-zinc-900 transition-colors"
                >
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <Avatar
                      avatarUrl={requester.avatar_url}
