...
const Leaderboard = () => {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      ...
      if (activeTab === 'daily') {
        ...
        data = await getTotalPointsLeaderboard();
      } else if (activeTab === 'streak') {
        data = await getLongestStreakLeaderboard();
      }
  ...
  const getTrophyIcon = (index: number) => {
    ...
    if (index === 2) return <Trophy size={24} className="text-yellow-600" />;
    return <span className="text-lg font-bold text-zinc-500 w-8 text-center">{index + 1}</span>;
  };
  ...
  const LeaderboardList = () => {
    ...
    if (leaderboardData.length === 0) {
      return <div className="text-center text-zinc-500 py-8">No data available yet. Check back tomorrow!</div>;
    }
      ...
      <div className="space-y-2">
        {leaderboardData.map((user, index) => (
          <div key={user.id} className={`flex items-center justify-between p-3 rounded-lg ${
            index < 3 ? 'bg-zinc-800 border-l-4 border-yellow-500' : 'bg-zinc-900'
          }`}>
            <div className="flex items-center gap-4">
              <div className="w-8 flex justify-center">{getTrophyIcon(index)}</div>
              <span className="font-medium text-white text-sm sm:text-base">{user.username}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-base sm:text-lg font-bold text-emerald-400">{user.score}</span>
              {activeTab === 'daily' && <TrendingUp size={16} className="text-emerald-500" />}
              {activeTab === 'total' && <Star size={16} className="text-yellow-500" />}
              {activeTab === 'streak' && <TrendingUp size={16} className="text-red-500" />}
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
        ...
        <div className="flex items-center gap-2 p-1 bg-zinc-900 rounded-lg self-start sm:self-center">
