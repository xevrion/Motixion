...
export const Layout: React.FC<LayoutProps> = ({ children, currentView, setView }) => {
    ...
    return (
      ...
      >
        <Icon
          ...
          className={isActive ? 'text-emerald-500' : 'text-zinc-500 group-hover:text-zinc-700 dark:group-hover:text-zinc-300'}
        />
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 overflow-hidden font-sans">

      {/* Mobile Header */}
      <div className="md:hidden flex items-center justify-between p-4 border-b border-zinc-300 dark:border-zinc-800 bg-white dark:bg-zinc-950 z-50">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-3">
          ...
          <h1 className="font-bold text-lg tracking-tight text-zinc-900 dark:text-white">Motixion</h1>
        </motion.div>
        <ThemeToggle mobile />
      </div>

      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-64 flex-shrink-0 border-r border-zinc-300 dark:border-zinc-800 bg-white dark:bg-zinc-950 flex-col">
        <div className="p-6 flex flex-col h-full">
          ...
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3 mb-8">
            ...
            <div>
              <h1 className="font-bold text-xl tracking-tight text-zinc-900 dark:text-white leading-none">Motixion</h1>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1 font-medium">v1.0</p>
            </div>
          </motion.div>
          ...
          <motion.button
            ...
            className="w-full bg-emerald-500 hover:bg-emerald-400 text-white font-bold py-3 px-4 rounded-xl shadow-lg shadow-emerald-900/20 flex items-center justify-center gap-2 transition-all mb-6"
          >
      ...
      {/* Main Content */}
      <main className="flex-1 overflow-y-auto bg-zinc-100 dark:bg-zinc-950 pb-20 md:pb-0">
        <motion.div
          key={currentView}
      ...
      >
        ...
        <motion.button whileTap={{ scale: 0.9 }} onClick={() => setView(ViewState.LOG)} className="flex flex-col items-center justify-center gap-1 -mt-6">
          ...
          <span className="text-xs font-medium text-emerald-600 dark:text-emerald-400 mt-1">Log</span>
        </motion.button>
